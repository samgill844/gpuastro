...
def _rv(time, RV, RV_err, RV_jitter, t_zero, period, K1, w, e, V0, dV0, VS, VC, radius_1, k, incl, ld_law_1, ldc_1, t_ecl_tolerance, Accurate_t_ecl,  Accurate_Eccentric_Anomaly, E_tol, RV_, N_start):
    ...
    for i in range(time.shape[0]):
        ...
        if RV_jitter==-99.: RV_[N_start*time.shape[0] + i] = K1*math.cos(nu + w)              + V0  + dV0*(time[i] - t_zero)
        else:
            wt = 1./(RV_err[i]**2 + RV_jitter**2)
            RV_[N_start] -= 0.5*(( (K1*math.cos(nu + w)              + V0  + dV0*(time[i] - t_zero)) - RV[i])**2*wt - math.log(wt))             



@numba.njit(fastmath=True)
def rv(time, RV=-99.*np.ones(2), RV_err=-99.*np.ones(2), RV_jitter=-99., t_zero=0.0, period=1.0, K1=10., fs=0.0, fc=0.0, V0=0.0, dV0=0.0, VS=0.0, VC=0.0, radius_1=0.2, k=0.2, incl=90., ld_law_1 = 0, ldc_1 = np.array([0.8,0.8]), t_ecl_tolerance=1e-5, Accurate_t_ecl=False,  Accurate_Eccentric_Anomaly=False, E_tol=1e-9):
    # Unpack args
    w = math.atan2(fs, fc)
    e = fs*fs + fc*fc
    ...
    if RV_jitter==-99.:  RV_ = np.empty(time.shape[0], dtype = np.float64)
    else:                RV_ = np.zeros(1, dtype = np.float64)

    # Call the kernel
    _rv(time, RV, RV_err, RV_jitter, t_zero, period, K1, w, e, V0, dV0, VS, VC, radius_1, k, incl, ld_law_1, ldc_1, t_ecl_tolerance, Accurate_t_ecl,  Accurate_Eccentric_Anomaly, E_tol, RV_, 0)
...
if numba.cuda.is_available():
    ...
    def _rv_gpu(time, RV, RV_err, RV_jitter, t_zero, period, K1, fs, fc, V0, dV0, VS, VC, radius_1, k, incl, ld_law_1, ldc_1, t_ecl_tolerance, Accurate_t_ecl,  Accurate_Eccentric_Anomaly, E_tol, RV_):
        ...
        # Call kernel
        _rv(time, RV, RV_err, RV_jitter[j], t_zero[j], period[j], K1[j], w, e, V0[j], dV0[j], VS[j], VC[j], radius_1[j], k[j], incl_, ld_law_1, ldc_1, t_ecl_tolerance, Accurate_t_ecl,  Accurate_Eccentric_Anomaly, E_tol, RV_, j)
