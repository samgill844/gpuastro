...
if numba.cuda.is_available() : __all__ = ['lc', 'lc_loglike', 'lc_gpu', 'lc_loglike_gpu']
...
def __lc(time, mag, mag_err, mag_zp, mag_jitter,\
    ...
    for i in range(time.shape[0]):
        ...
        L -= 0.5*((l - mag[i])**2*wt - math.log(wt))
    return L


@numba.cuda.jit
def d__lc(time, mag, mag_err, mag_zp, mag_jitter,\
        radius_1, k, w, e, incl,\
        period, t_zero,\
        ldc_1_1, ldc_1_2, ld_law_1,\
        ld_1_func, N_BATMAN, FAC_BATMAN,\
    ...
    if (i < time.shape[0]):
        ...
        # Now add it 
        numba.cuda.atomic.add(L, 0, - 0.5*((l - mag[i])**2*wt - math.log(wt)))
...
def lc_loglike(time,
               ...
               radius_1=0.2, k=0.2, fs=0.0, fc=0.0, incl=90.,
               period=1.0, t_zero=0.0,
               ldc_1_1=0.8, ldc_1_2=0.8, ld_law_1 = 0,
               ld_1_func=-1, N_BATMAN=1000, FAC_BATMAN=5.0e-4,  SBR = 0.0, light_3 = 0.0,
               t_ecl_tolerance=1e-5, Accurate_t_ecl=False,  Accurate_Eccentric_Anomaly=False, E_tol=1e-9):
    '''
    Return the lightcurve model. 
    ...
    # Allocate the LC array
    return __lc(time,mag, mag_err, mag_zp, mag_jitter, radius_1, k, w, e, incl, period, t_zero, ldc_1_1, ldc_1_2, ld_law_1, ld_1_func, N_BATMAN, FAC_BATMAN,  SBR, light_3,    t_ecl_tolerance, Accurate_t_ecl,  Accurate_Eccentric_Anomaly, E_tol)
...
if numba.cuda.is_available():
    ...
    def lc_loglike_gpu(time,\
        ...
        # make the call
        d__lc[blocks, threads_per_block ](time,mag, mag_err, mag_zp, mag_jitter, radius_1, k, w, e, incl, period, t_zero, ldc_1_1, ldc_1_2, ld_law_1, ld_1_func, N_BATMAN, FAC_BATMAN,  SBR, light_3,    t_ecl_tolerance, Accurate_t_ecl,  Accurate_Eccentric_Anomaly, E_tol, L)
